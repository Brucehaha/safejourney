	...
	//Check the form if submit by post
		if (isset($_POST["searchBtn"])) {
			...
			if(!empty($strInputSuburb) || !empty($strInputStreet))
			{
				...
				//$querySql1 = "select * from Infringement where suburb like '%$strInputSuburb%' and street1 like '%$strInputStreet%'";
				...
				if(!$count==0){
					...
					while($Row = mysqli_fetch_row($result1)){
						echo "<tr><td>".$i."</td><td>Intersection of ".$Row[1]." and ".$Row[2]."</td><td>".$Row[3]."</td><td>".$Row[4]."</td><td>".$Row[5]."</td></tr>";
						$i++;
					}
					echo "</table>";
					echo "</div>";
